...
import { getPharmacyBrandingSettings, getPharmacyThemeSettings } from './settingsService'
...
describe('settingsService theme settings', () => {
  ...
  it('loads report branding settings without creating a pharmacy_settings row', async () => {
    getCurrentSupabaseUser.mockResolvedValue({ id: 'user-1' })
    const userQuery = {
      select: vi.fn(() => userQuery),
      eq: vi.fn(() => userQuery),
    ...
    const settingsQuery = makeSettingsQuery({
      data: {
        pharmacy_name: 'West Point Chemist',
        logo_url: 'https://example.test/logo.png',
        report_footer: 'Generated by HealthFlow',
      },
    })
    from.mockImplementation((table) => {
      if (table === 'users') return userQuery
    ...
    const settings = await getPharmacyBrandingSettings()
    ...
    expect(settingsQuery.select).toHaveBeenCalledWith(
      'pharmacy_name,logo_url,slogan,receipt_footer,report_footer,theme_primary_color,theme_secondary_color,theme_accent_color'
    )
    expect(settingsQuery.eq).toHaveBeenCalledWith('organization_id', 'org-1')
    expect(settingsQuery.insert).not.toHaveBeenCalled()
  })
