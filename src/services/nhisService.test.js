...
describe('CLAIM-it export helpers', () => {
  ...
  it('keeps the generated CXF and reports partial success when the export record cannot be saved, without regenerating', async () => {
    const submittedClaim = {
      ...claim,
      status: 'submitted',
      organization_type: 'pharmacy',
    ...
    const recording = { artifactSha256: 'a'.repeat(64), pendingChunkIndexes: [0], chunks: [{ claimIds: ['claim-1'], fingerprints: {} }] }
    recordCxfExport.mockResolvedValueOnce({ claimNumber: '', warnings: ['EXPORT_AUDIT_NOT_RECORDED'], recording })
    const warnings = []
    const preparedReadiness = await prepareNhisClaimsExport(options)
    const count = await exportNhisClaimsFile({ ...options, preparedReadiness, exportRunId: 'run-1', onExportWarnings: (list) => warnings.push(...list) })

    // The file was produced and downloaded; the failed record is reported (with a retry handle), not hidden or thrown.
    expect(count).toBe(1)
    expect(clickSpy).toHaveBeenCalledTimes(1)
    expect(warnings.find((warning) => warning.recording)?.recording).toBe(recording)
    expect(recordCxfExport).toHaveBeenCalledWith(expect.any(Array), expect.any(Uint8Array), undefined, { exportRef: 'run-1' })
    // Recording used the already generated bytes; the claims were loaded once and nothing was rebuilt for the record.
    expect(claimsSelect).toHaveBeenCalledTimes(1)
    clickSpy.mockRestore()
  })
