...
const listBranchSyncSetupOptions = async (adminClient: ReturnType<typeof createAdminClient>) => {
  const { data: organizations, error: organizationsError } = await adminClient
    .from('organizations')
  ...
  if (organizationIds.length > 0) {
    const { data: branchRows, error: branchesError } = await adminClient
      .from('branches')
...
const registerBranchSyncClient = async (
  adminClient: ReturnType<typeof createAdminClient>,
  payload: Record<string, unknown>
) => {
  const organizationId = normalizeText(payload.organizationId)
  const requestedBranchId = normalizeText(payload.branchId)
  const branchAutoGenerated = payload.branchAutoGenerated === true
  const name = normalizeText(payload.name) || 'Branch server'
  const branchNameInput = normalizeText(payload.branchName)
  const token = normalizeText(payload.branchSyncToken) || generateBranchSyncToken()

  if (!organizationId) {
    throw new Error('Organization is required.')
  }

  if (requestedBranchId && !UUID_PATTERN.test(requestedBranchId)) {
    throw new Error('Branch id must be a valid UUID.')
  }

  if (token.length < 32) {
    throw new Error('Branch sync token must be at least 32 characters.')
  }
  ...
  let branch: { id: string; name?: string | null; organization_id: string } | null = null
  let branchCreated = false

  if (requestedBranchId) {
    const { data: requestedBranch, error: requestedBranchError } = await adminClient
      .from('branches')
    ...
    if (requestedBranchError) {
      throw requestedBranchError
    }

    branch = requestedBranch
  }

  if (!branch && branchAutoGenerated) {
    ...
    branch = existingBranches?.[0] || null
  }

  if (!branch && branchAutoGenerated) {
    const branchId = requestedBranchId || crypto.randomUUID()
    const branchName = branchNameInput || `${normalizeText(organization.name) || 'Main'} Main branch`

    const { data: insertedBranch, error: insertBranchError } = await adminClient
      .from('branches')
      .insert([
        {
          id: branchId,
  ...
  if (!branch) {
    throw new Error('Branch does not belong to the selected organization.')
  }

  const { data: confirmedBranch, error: branchError } = await adminClient
    .from('branches')
  ...
  const { data: syncClientId, error: rpcError } = await adminClient.rpc(
    'create_branch_sync_client',
    {
      p_name: name,
...
Deno.serve(async (request) => {
  ...
  try {
    ...
    if (
      ...
      if (action === 'list_branch_sync_setup_options') {
        return json(await listBranchSyncSetupOptions(adminClient))
      }
