-- Include authorized membership identifiers and tenant-scoped recycle snapshots.
...
begin
  ...
  with tenant_logs as materialized (
    ...
    select a.*, actor.full_name as actor_name,
      ...
      case when c.id is not null then jsonb_build_object('surname', c.surname, 'other_names', c.other_names, 'claim_number', c.claim_number, 'prescription_reference', c.prescription_reference, 'member_no', c.member_no) when archived.id is not null then jsonb_build_object(
        'surname', archived.snapshot->'record'->>'surname', 'other_names', archived.snapshot->'record'->>'other_names',
        'claim_number', archived.snapshot->'record'->>'claim_number', 'prescription_reference', archived.snapshot->'record'->>'prescription_reference',
        'member_no', archived.snapshot->'record'->>'member_no') end as claim_record,
      (c.id is null and archived.id is not null) as claim_is_archived,
      ...
      coalesce(c.branch_id::text, d.branch_id::text, s.branch_id::text,
        u.branch_id::text, nullif(a.details->>'branch_id', ''), nullif(archived.snapshot->'record'->>'branch_id', '')) as record_branch,
    ...
    left join public.deleted_records archived on a.kind = 'claim' and c.id is null
      and archived.entity_type = 'nhis_claim' and archived.entity_id = a.entity_id
      and archived.organization_id = p_organization_id
    ...
    select id, actor_user_id, actor_email, actor_name, actor_is_active,
      ...
      jsonb_build_object('type', kind, 'restricted', not can_identify,
        'fields', case when not can_identify then '{}'::jsonb else jsonb_strip_nulls(
          case kind
            when 'claim' then jsonb_build_object(
              ...
              'Membership number', coalesce(nullif(btrim(details->>'member_no'), ''), nullif(btrim(claim_record->>'member_no'), '')),
        ...
        'source', case when not can_identify then 'restricted'
          when details ?| array['member_no','patient_name','claim_number','prescription_reference','medicine_name','drug_name','name','sale_number','receipt_number','full_name','setting_name','setting','payment_reference','file_name','report_name'] then case when coalesce(claim_record, drug_record, sale_record, patient_record, staff_record) is null or claim_is_archived then 'snapshot' else 'snapshot_or_current' end
          when claim_is_archived then 'snapshot'
