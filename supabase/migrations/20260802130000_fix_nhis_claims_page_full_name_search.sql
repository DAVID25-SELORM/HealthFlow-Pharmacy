-- Fix NHIS claims list search: typing a full name (e.g. "Ayim Emma") matched
-- nothing because surname and other_names are separate columns, so neither
-- one alone contains the full two-word search term. Adds a match against
-- the concatenated full name in both column orders, on top of (not instead
-- of) every existing single-column match — purely additive, no existing
-- match narrowed. This mirrors the offline/branch-server search matcher
-- (nhisClaimMatchesListFilters in src/services/nhisService.js), which
-- already builds surname+other_names / other_names+surname concatenations.
-- No other NHIS behavior changes: pricing, tariffs, CCC, claim totals, and
-- export/validation rules are untouched.
  ...
  filtered as (
    ...
    where
      ...
      and (
        ...
        or trim(coalesce(c.surname, '') || ' ' || coalesce(c.other_names, '')) ilike '%' || p.search_filter || '%'
        or trim(coalesce(c.other_names, '') || ' ' || coalesce(c.surname, '')) ilike '%' || p.search_filter || '%'
      )
  ),
  counts as (
    select
