-- Combined NHIS claim page RPC.
-- Returns the current page of claim summaries plus optional matching totals in
-- one Supabase call. SECURITY INVOKER is intentional so existing RLS policies
-- still control which organization/facility rows the signed-in user can see.
