-- Phase 3/4 active-medication intelligence: refill/remaining-balance context
-- plus neutral claims-risk scoring for authorized review.
-- Read-only; no CCC, pricing, tariff, duplicate-claim, export, submission,
...
drop function if exists public.check_nhis_active_medication_overlap(text, text, text, date, uuid, uuid, text, text, text);
...
begin
  ...
  active_matches as (
    select
      ...
      greatest(((matched_lines.dispensed_date + (matched_lines.coverage_days - 1))::date - v_service_date), 0)::integer as computed_remaining_days,
      case
        ...
        when ((matched_lines.dispensed_date + (matched_lines.coverage_days - 1))::date - v_service_date) >= 7
